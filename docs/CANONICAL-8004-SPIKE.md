...
default read. The clean, demo-safe path is the **RetinAI-local registry that mirrors the
...
> average — its only sybil defense is blocking the agent's own owner. RetinAI adds the
...
> canonical 8004 as a tagged `retinai` feedback entry, so 8004-native readers can filter
> `getSummary(agentId, [reviewGate], "retinai", "")` to get the sybil-resistant score."
...
- **The only write restriction:** `msg.sender` must NOT be the agent owner or an approved
  ...
  precisely the sybil hole RetinAI closes.**
...
| Aspect | RetinAI mock / interface / backend | Canonical (deployed on Base) | Match? |
|---|---|---|---|
...
- Consequence for reads: `getSummary(agentId, [reviewGate], "", "")` returns the **aggregate
  of all RetinAI reviews** — which is actually *fine and even nice* (it's the human-gated
...
reputation.giveFeedback(
    ...
    0,                        // valueDecimals
    "retinai",              // tag1  -> lets 8004 readers filter to human-gated feedback
    "",                       // tag2
    "",                       // endpoint
    "",                       // feedbackURI
...
`getSummary(agentId, [address(reviewGate)], "retinai", "")`.
...
2. **New contract** `CanonicalReviewGate` (or a `canonical` mode flag) that, after the nullifier
   dedup, calls `canonicalReputation.giveFeedback(agentId, int128(score), 0, "retinai", "",
...
3. **Backend**: a second ABI fragment + a `--canonical`/env switch. When canonical:
   - read score via `getSummary(agentId, [reviewGate], "retinai", "")` and parse
