# HumanRank — FROZEN SPEC (single source of truth)
...
self-review. HumanRank gates reputation **writes** by World ID nullifier — **one human, one
...
```json
{
  ...
  "agents": [
    {
      "agentId": 1,
      "ensName": "research-agent.humanrank.eth",
      "wallet": "0x...",
...
3. **Hero comparison** `/compare/[id]` — side by side **Raw ERC-8004 score (5.0 ★, farmed)**
   vs **HumanRank score (1.0 ★)**, with a "Replay sybil attack" button that fires the
